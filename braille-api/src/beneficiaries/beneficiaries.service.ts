...
const ALUNO_LISTA_SELECT = {
  ...
  dataNascimento: true, telefoneContato: true, tipoDeficiencia: true,
...
const ALUNO_EXPORT_SELECT = {
  ...
  statusAtivo: true, criadoEm: true, corRaca: true,
} as const;
...
export class BeneficiariesService {
  ...
  async exportToXlsx(query: QueryBeneficiaryDto): Promise<Buffer> {
    ...
    const alunos = await this.prisma.aluno.findMany({
      where, orderBy: { nomeCompleto: 'asc' }, select: ALUNO_EXPORT_SELECT,
    });
  ...
  async remove(id: string, auditUser?: AuditUser) {
    ...
    this.auditService.registrar({
      ...this.toAuditMeta(auditUser),
      entidade: 'Aluno', registroId: id, acao: AuditAcao.EXCLUIR,
      oldValue: { statusAtivo: true }, newValue: { statusAtivo: false },
    });
    return result;
  }
  ...
  async restore(id: string, auditUser?: AuditUser) {
    ...
    this.auditService.registrar({
      ...this.toAuditMeta(auditUser),
      entidade: 'Aluno', registroId: id, acao: AuditAcao.RESTAURAR,
      oldValue: { statusAtivo: false }, newValue: { statusAtivo: true },
    });
    return result;
  }
  ...
  async removeHard(id: string, auditUser?: AuditUser) {
    ...
    this.auditService.registrar({
      ...this.toAuditMeta(auditUser),
      entidade: 'Aluno', registroId: id, acao: AuditAcao.ARQUIVAR,
      oldValue: { excluido: false }, newValue: { excluido: true },
    });
    return result;
  }
  ...
  private readonly COR_RACA_MAP: Record<string, string> = {
    ...
    'não declarado': 'NAO_DECLARADO', 'nao declarado': 'NAO_DECLARADO',
  ...
  async importFromSheet(buffer: Buffer, auditUser?: AuditUser): Promise<{
    importados: number;
    ignorados:  number;
    ...
    const headers: string[] = (rawRows[1] as unknown[]).map(h => String(h ?? '').trim());
    const dataRows = rawRows.slice(2);

    const rows: Record<string, unknown>[] = dataRows.map(row => {
      const obj: Record<string, unknown> = {};
      headers.forEach((key, idx) => { obj[key] = (row as unknown[])[idx] ?? ''; });
      return obj;
    });
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        erros.push({ linha, documento: documentoVisivel, motivo: `DataNascimento inválida: "${String(dataNascimentoRaw)}". Use DD/MM/AAAA ou AAAA-MM-DD` });
        continue;
      }

      const rTipo   = this.normalizarEnum(String(row['TipoDeficiencia'] ?? '').trim(), this.TIPO_DEFICIENCIA_MAP, ['CEGUEIRA_TOTAL', 'BAIXA_VISAO', 'VISAO_MONOCULAR'], 'TipoDeficiencia');
      if (rTipo.erro)  { erros.push({ linha, documento: documentoVisivel, motivo: rTipo.erro  }); continue; }

      const rCausa  = this.normalizarEnum(String(row['CausaDeficiencia'] ?? '').trim(), this.CAUSA_DEFICIENCIA_MAP, ['CONGENITA', 'ADQUIRIDA'], 'CausaDeficiencia');
      if (rCausa.erro) { erros.push({ linha, documento: documentoVisivel, motivo: rCausa.erro }); continue; }

      const rPref   = this.normalizarEnum(String(row['PrefAcessibilidade'] ?? '').trim(), this.PREF_ACESSIBILIDADE_MAP, ['BRAILLE', 'FONTE_AMPLIADA', 'ARQUIVO_DIGITAL', 'AUDIO'], 'PrefAcessibilidade');
      ...
      const rCorRaca   = this.normalizarEnum(corRacaRaw, this.COR_RACA_MAP, ['BRANCA', 'PRETA', 'PARDA', 'AMARELA', 'INDIGENA', 'NAO_DECLARADO'], 'CorRaca');
      if (rCorRaca.erro && corRacaRaw !== '') { erros.push({ linha, documento: documentoVisivel, motivo: rCorRaca.erro }); continue; }
    ...
    for (const aluno of validos) {
      if ((aluno.cpf && cpfsExistentes.has(aluno.cpf as string)) || (aluno.rg && rgsExistentes.has(aluno.rg as string))) {
        erros.push({ linha: 0, documento: String(aluno.cpf || aluno.rg), motivo: 'CPF ou RG já cadastrado no sistema' });
        ignorados++;
      } else {
    ...
    if (paraInserir.length > 0) {
      ...
      try {
        ...
        throw new InternalServerErrorException('Falha na importação dos dados. Tente novamente ou verifique o arquivo.');
      }
      ...
      Promise.resolve().then(async () => {
        for (const aluno of paraInserir) {
          ...
          }).catch(() => { /* auditoria nunca deve derrubar a operação */ });
        }
      });
    }
